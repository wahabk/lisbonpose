...
if __name__ == '__main__':
    ...
    vp3 = ransac_vanishing_point(edgelets2, num_ransac_iter=2000,
                                 threshold_inlier=5)
    vp3 = reestimate_model(vp3, edgelets3, threshold_reestimate=5)
    vis_model(image, vp3) # Visualize the vanishing point model

    edgelets4 = remove_inliers(vp3, edgelets3, 10)
    vp4 = ransac_vanishing_point(edgelets3, num_ransac_iter=2000,
                                 threshold_inlier=5)
    vp4 = reestimate_model(vp4, edgelets4, threshold_reestimate=5)
    vis_model(image, vp4) # Visualize the vanishing point model

    clip_factor = 3
    
    choice = int(input('Which vp to use?'))

    if choice == 2:
        warped_img = compute_homography_and_warp(image, vp1, vp2,
                                         clip_factor=clip_factor)
    elif choice == 3:
        warped_img = compute_homography_and_warp(image, vp1, vp3,
                                         clip_factor=clip_factor)
    elif choice == 4:
        warped_img = compute_homography_and_warp(image, vp1, vp4,
                                         clip_factor=clip_factor)

    
    plt.subplot(), plt.imshow(warped_img, cmap = 'gray')
    plt.title('Warped Image'), plt.xticks([]), plt.yticks([]), plt.show()
